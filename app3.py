...
def split_text_smartly(text):
    ...
    # 判斷是否為「數字編號」開頭 (例如 1 ..., 2 ...)
    if re.search(r'(?m)^\s*\d+\s', text):
        # 1. 依照數字切割 (包含處理數字後面的點或空白)
        raw_segments = re.split(r'(?m)^\s*\d+[.:]?\s+', text)
        ...
        if segments:
            ...
            clean_full_text = re.sub(r'(?m)^\s*\d+[.:]?\s+', '', clean_full_text)
            clean_full_text = re.sub(r'(?i)\b[AB]\s*:', '', clean_full_text)
            clean_full_text = clean_full_text.replace('\n', ' ')
            clean_full_text = re.sub(r'\s+', ' ', clean_full_text).strip()
            
            segments.append("🌟 Full Text Review: " + clean_full_text)
        
        return segments

    else:
        # --- 原有邏輯 (無數字編號時，維持標點符號分段) ---
